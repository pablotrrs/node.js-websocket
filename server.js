...
Object.entries(connections).forEach(([key, settings]) => {
    ...
    const server = new WebSocket.Server({ port: settings.port }, () => { console.log(`WS Server is listening at ${ settings.port }`);})
                    .on('connection', (ws) => {
                        ws.on('message', data => {
                            if (ws.readyState !== ws.OPEN) return;
                            if (typeof data === 'object') {
                                ...
                                connection.sensors = data.split(",").reduce((acc, timer) => {
